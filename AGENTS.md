...
`src/liblt/` subsystem map:
  - `LTE/`      — engine core: type system, serializer, scripting (LTSL),
               ...
               Lexer (`Lexer.h`/`Lexer.cpp` — Phase 1 compiler frontend).
...
### A.16 Phase 1 Lexer — Token Scanner (2026-08-25)

Implements Phase 1 of `docs/LTSL-MIGRATION-PLAN.md`. **Suite: 1161 checks /
0 failures. Compile gate = PASS, 157 files, empty allowlist.**

- [x] **`src/liblt/LTE/Lexer.h`** — `TokenKind` enum (TOK_INT through
      TOK_UNKNOWN), `Token` struct (kind, value, line, column, length),
      `LexError` struct, `Lexer` class. Tokens dropped from the planned enum:
      `TOK_MEMBER` (dot-chain stays separate tokens — parser rewrites),
      `TOK_SEMICOLON`, `TOK_ARROW`, `TOK_IN`, `TOK_CONTINUE` (none present
      in corpus; `TOK_SIZEOF`/`TOK_TYPEOF` confirmed absent from engine).
- [x] **`src/liblt/LTE/Lexer.cpp`** — Full implementation:
  - Character-level scanning (`Peek`/`Peek2`/`Advance`/`AtEnd`)
  - `#` = single-line comment only (16 block-disabling occurrences must be
    rewritten in the pending `#` migration before this lexer is wired in as
    the sole tokenization path)
  - `parenDepth` counter: inside `()`/`[]`, newlines are consumed but no
    NEWLINE/INDENT/DEDENT emitted — enables multi-line expressions
  - `hasTokensOnLine` flag: NEWLINE only emitted when real content appeared
    on the line — comment-only and blank lines produce no NEWLINE token
  - Python-style indent stack (`Vector<int>`, starts `[0]`): INDENT emitted
    when indent increases, DEDENT when it decreases; hard error on
    mismatched dedent (`ReportError`)
  - Tab treated as 4 spaces for indent measurement
  - `MeasureIndent()` saves/restores `pos`/`column` — measures without
    consuming; `SkipHorizontalWhitespace()` called after
    `EmitIndentTokens()` to advance
  - `Emit()` helper marks `hasTokensOnLine` for non-structural tokens
  - All operators: `+` `-` `*` `/` `%` `==` `!=` `<` `>` `<=` `>=` `&&`
    `||` `!` `=` `+=` `-=` `*=` `/=`
  - All delimiters: `(` `)` `[` `]` `,` `:` `.`
  - All keywords: `var` `ref` `static` `function` `return` `if` `else`
    `while` `for` `switch` `otherwise` `type` `cast` `break` `desc` `block`
    `address` `deref` `true` `false` `null`
  - `@` prefix debug-print operator → `TOK_AT`
  - Literals: integers, floats, single-quoted strings, double-quoted strings,
    booleans, null
  - Postfix operators (`.!` `.++` `.--`) lexed as separate tokens:
    `IDENT DOT NOT` / `IDENT DOT PLUS PLUS` — parser handles rewriting
  - Error on unterminated string, unexpected character
- [x] **`tests/TestLexer.cpp`** — 35 unit tests covering:
  - Basic types (int, float, string, bool, null, identifier)
  - All keywords recognized correctly
  - All operators (arithmetic, comparison, logical, assignment)
  - All delimiters including `@`
  - Single-line comments and trailing comments
  - Newline handling, blank lines suppressed (no NEWLINE token)
  - Indentation: simple, nested, mismatched dedent (hard error), tabs as indent
  - Multi-line parens and nested parens suppress NEWLINE
  - Line/column tracking across operators, strings, multi-line input
  - Error cases: unterminated string, unknown character, EOF
  - Postfix operators as separate tokens
  - Complex expressions, function declarations, method calls

### What the new lexer fixes that the old interpreter could not

- **Hard error on mismatched indentation** — old interpreter silently accepted
  dedents to non-existent levels (the HUD.lts/SystemPopulate.lts class of bug)
- **Precise line/column on every token** — enables targeted error messages in
  future Phase 3 type checker
- **Single-line `#` semantics** — once the 16 block-disabling occurrences are
  rewritten, `#` is unambiguous; no silent block deactivation
- **Correct comment-line suppression** — old interpreter emitted spurious
  NEWLINE tokens for comment-only lines, confusing downstream tools
